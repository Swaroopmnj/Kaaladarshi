...
import { panchakaTypeFromNakshatra, getPanchakaVerdict, PANCHAKA_EXCEPTIONS, calculatePanchakaRahita, extractTithiNumber, varaNumberFromName } from './lib/panchaka';
...
import { getBhadraDayVerdict, getBhadraIntervals } from './lib/bhadra';
...
export default function App() {
  const [tab, setTab] = useState<'finder' | 'panchang' | 'personal' | 'fullReport' | 'guide'>('panchang');
  ...
  const [reportCharts, setReportCharts] = useState<Record<string, BirthChart>>({});
  ...
  function generateReport(key: string, r: EnrichedDay, forCity: ReturnType<typeof resolveCity>) {
    const windows = getGoodTimeWindows(r.raw.panchang, { latitude: forCity.latitude, longitude: forCity.longitude }, forCity.timezone);
    const anchor = windows[0]?.start ?? r.raw.panchang.sunrise;
    const realInstant = toRealInstant(anchor, forCity.timezone);
    const chart = computeRashiChart(realInstant, { latitude: forCity.latitude, longitude: forCity.longitude });
    setReportCharts((prev) => ({ ...prev, [key]: chart }));
  }
